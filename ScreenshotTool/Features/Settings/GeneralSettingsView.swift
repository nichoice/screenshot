...
struct GeneralSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 20) {
            SettingsSectionCard(
                title: "应用行为",
                description: "SnapPii 会以无 Dock 图标的方式后台常驻；这里可以决定是否额外保留菜单栏入口。"
            ) {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    Toggle("开机启动", isOn: Binding(
                        get: { viewModel.launchAtLoginStatus },
                        set: { enabled in
                            try? viewModel.setLaunchAtLogin(enabled)
                        }
                    ))
            ...
            SettingsSectionCard(
                ...
                VStack(alignment: .leading, spacing: 12) {
                    statusRow("软件版本", value: viewModel.appVersionDisplayName)
                    statusRow("后台驻留", value: "已启用")
                    statusRow("Dock 图标", value: "已隐藏")
                    statusRow("菜单栏入口", value: viewModel.appPreferences.showsMenuBarIcon ? "已显示" : "已隐藏")
                    statusRow("开机启动", value: viewModel.launchAtLoginStatus ? "已启用" : "未启用")
